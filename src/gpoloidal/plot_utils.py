...
rcparam_journal= {   
    # --- 図全体の設定 ---
    "figure.figsize": (3., 2.5),       # 図のサイズ（インチ単位）
    "figure.dpi": 200,                  # 画面上の解像度
    "savefig.dpi": 300,                 # 保存時の解像度（300dpi以上が推奨）

    # --- フォント設定 ---
    # Nature ではサンセリフ体（例: Helvetica）が好まれる
    "font.family": "sans-serif",
    "font.sans-serif": ["Arial"],
    "font.size": 8,                    # 基本フォントサイズ
    "axes.labelsize": 8,               # 軸ラベルのフォントサイズ
    "axes.titlesize": 7,               # タイトルのフォントサイズ
    "xtick.labelsize": 6,               # X軸目盛りのフォントサイズ
    "ytick.labelsize": 6,               # Y軸目盛りのフォントサイズ
    "legend.fontsize": 6,               # 凡例のフォントサイズ


    # --- 線とマーカーの設定 ---
    "lines.linewidth": 1.,             # 線の太さ
    "lines.markersize": 5,              # マーカーサイズ

    # --- 軸・目盛りの設定 ---
    "axes.linewidth": 0.6,              # 軸枠線の太さ
    "xtick.major.width": 0.5,           # X軸主要目盛りの線幅
    "ytick.major.width": 0.5,           # Y軸主要目盛りの線幅
    "xtick.minor.width": 0.3,           # X軸補助目盛りの線幅
    "ytick.minor.width": 0.3,           # Y軸補助目盛りの線幅
    "xtick.direction": "in",            # X軸目盛りの向き（"in", "out", "inout"）   
    "ytick.direction": "in",            # Y軸目盛りの向き（"in", "out", "inout"）
    #"xtick.minor.size": 3,              # X軸補助目盛りの長さ
    #"ytick.minor.size": 3,              # Y軸補助目盛りの長さ
    #"xtick.major.size": 5,              # X軸主要目盛りの長さ
    #"ytick.major.size": 5,              # Y軸主要目盛りの長さ

    # --- グリッド ---
    "axes.grid": True,                 # グリッド表示（必要に応じて True に変更）
    "grid.linestyle": "--",              # グリッド線のスタイル
    "grid.linewidth": 0.3,              # グリッド線の太さ
    "grid.alpha": 1,                  # グリッド線の透明度
...
JOURNAL_MODE =False
cm_data_R = [[ 0.23605636, 0.03529748, 0.06943744],
...
cm_data_B = [[ 0.0931763 , 0.11117333, 0.26151239],
...
cm_data =  cm_data_B+cm_data_R[::-1] 
cmocean_balance = ListedColormap(cm_data, name="local_balance")
cmocean_balance_r = ListedColormap(cm_data[::-1], name="local_balance_r")

def journal_mode():
    global JOURNAL_MODE
    JOURNAL_MODE = True
    plt.rcParams.update(rcparam_journal)
